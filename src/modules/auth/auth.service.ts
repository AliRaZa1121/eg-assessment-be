...
export default class AuthService {
    ...
    async refreshAccessToken(
        ...
        const auth: UserAuthInterface = await this.getSession(refToken);
    ...
    async destroySession(token: string): Promise<boolean> {
        const Auth: UserAuthInterface = await this.getSession(token);
        if (!Auth) return false;
        await this._redisCacheService.Delete(token);
        return true;
    }

    async getSession(token: string): Promise<UserAuthInterface> {
    ...
    private async _validateAccessToken(accessToken: string) {
        return await this._jwtService.verify(accessToken, {
            secret: this._configService.get('ACCESS_TOKEN_SECRET'),
        });
    }
